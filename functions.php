...
/** 
  * @desc generate array to select options
  * @param array $object - list of options
  * @param int $selected - value of selected value
  * @param string $identification - id of select
  * @param boolean $empty - either to include or not empty values
  * @return nothing
*/  
function createSelect ($object, $selected, $identification, $empty = true, $valuecheck = false) {
    global $tpl;
    ...
    if (gettype($object) == 'array') {
        ...
        foreach ($object as $key => $value) {
            ...
            if ($empty && ($key == 'null' || $key === '' || $value === 'null' || $value === '')) {
                continue;
            }
    ...
    $tpl->setArray(strtoupper($identification), $list);
}
...
/** 
  ...
  * @return nothing
*/  
function createSelectOptions ($object, $selected, $name = "", $class = "", $type = "", $valuecheck = false) {
    ...
    if (gettype($object) == 'array') {
        ...
        foreach ($object as $key => $value) {
            ...
            $list .= sprintf('<option value="%s" %s>%s</option>', $key, $check == $selected ? 'selected="selected"' : '', $value);
        }
    }
    $list .= '</select>';
    return $list;
